...
export const App:React.FC = () => {
    ...
    useEffect(() => {
        ...
        client.onmessage = (message) => {
            ...
            if(data.method === ActionCodes.UnDraw){
                if(paths.current.length > 0){
                    paths.current = paths.current.slice(0,paths.current.length-1);
                }
                return reDraw();
            }
            if (data.method === ActionCodes.RestartDraw){
                paths.current = [];
                return reDraw();
                //return clearWholeThing({preventDefault:() => {}})
            }

            if(data.method === ActionCodes.Draw){
                contextRef.current.beginPath();
                const context = canvasRef.current.getContext("2d");
                //context.strokeStyle = path.color;
                context.strokeStyle = data.draw.color;
                context.lineWidth = data.draw.size;
                // @ts-ignore
                data.draw.path.forEach(coordinate => {
